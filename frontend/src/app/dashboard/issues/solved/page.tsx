...
export default function SolvedPage() {
    ...
    const filteredIssues = useMemo(() => {
        return allIssues.filter(i => {
            if (i.status !== 'fixed') return false;
            if (searchQuery) {
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto p-6 space-y-6">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">Solved Issues</h1>
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-24 text-center">
                            <CheckCircle2 className="h-10 w-10 text-gray-300 mb-4" />
                            <h3 className="text-sm font-medium text-gray-600 mb-1">No solved issues yet</h3>
                            <p className="text-xs text-gray-500">Issues marked as fixed will appear here</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filteredIssues.map(issue => {
                                        ...
                                        const fixedAt = issue.updated_at ? new Date(issue.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
                                        ...
                                        return (
                                            <tr key={issue.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setSidebarId(issue.id)}>
                                                ...
                                                <td className="px-5 py-4">
                                                    <div className="font-medium text-gray-900 line-through text-gray-500">{issue.title}</div>
                                                    <div className="text-xs text-gray-400 mt-0.5">in {filePath}</div>
                                                </td>
                                                <td className="px-5 py-4">
                                                ...
                                                <td className="px-5 py-4">
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-green-700 bg-green-50 border border-green-200">
                                                        <CheckCircle2 className="h-3.5 w-3.5" /> Fixed {fixedAt}
                                                    </span>
                                                </td>
                                            </tr>
                                        );
